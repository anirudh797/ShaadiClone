package com.anirudh.shaadi.data.localDb

import android.content.Context
import androidx.room.Database
import androidx.room.Room
import androidx.room.RoomDatabase

@Database(entities = [EntityProfileInfo::class], version = 1)
abstract class UserMatchesDb : RoomDatabase() {
    abstract fun profilesDao(): ProfileInfoDao

    companion object {
        @Volatile
        private var instance: UserMatchesDb? = null
        private val LOCK = Any()

        operator fun invoke(context: Context): UserMatchesDb {
            return instance ?: synchronized(LOCK) {
                instance ?: buildDatabase(context).also {
                    instance = it
                }
            }
        }

        private fun buildDatabase(context: Context) =
            Room.databaseBuilder(context.applicationContext, UserMatchesDb::class.java, "itemDB")
                .build()
    }
}